...
with open('./7/input.txt', 'r') as input:
  ...
  for line in input:
    ...
    for card in cards[:-1]:
      ...
      matches = re.findall(fr'{card}|J', hand)
      ...
      if len(matches) == 3:
        ...
        if vwin == 1 and ('J' not in hand or 'J' not in matches): new_v = 4
        elif vwin == 3:
          if matches.count('J') == 2: new_v = 3
          elif 'J' in matches: new_v = 4
          else: new_v = 3
        else: new_v = 3
      ...
      if len(matches) == 2:
        ...
        if vwin == 3:
          if 'J' not in hand or 'J' not in matches: new_v = 4
          else: new_v = 3
        elif vwin == 1 and ('J' not in hand or 'J' not in matches): new_v = 2
        else: new_v = 1
